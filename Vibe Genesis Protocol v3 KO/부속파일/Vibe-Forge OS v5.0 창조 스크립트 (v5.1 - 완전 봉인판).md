### **`[FORGE]` 최종 결과물: Vibe-Forge OS v5.0 창조 스크립트 (v5.1 - 완전 봉인판)**

```python
# /create_os.py (v5.1 - Refined by Symbiotic Weaver)
...
class WorkspaceManager:
    ...
    def save(self, interaction_id: str, filename: str, content: str) -> str:
        ...
        if "```html" in content or cleaned_content.strip().startswith("<!DOCTYPE html>"):
            ...
            filename += ".py"
        elif "```json" in content or (cleaned_content.startswith("{") and cleaned_content.endswith("}")):
             filename += ".json"
        else:
            filename += ".md"
        ...
        for mem in relevant_memories:
            ...
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    context_string += f"// Vibe: {metadata.get('user_vibe', 'N/A')}\\n"
...
class MemoryManager:
    def __init__(self, model_name='all-MiniLM-L6-v2', memory_dir="memory"):
        ...
        try:
            ...
            print("   인터넷 연결을 확인하거나, 모델 캐시 문제를 확인해주세요.")
            raise
...
def create_project(base_path="VIBE_FORGE_OS_v5.0"):
    ...
    if root.exists() and any(root.iterdir()):
        print(f"❌ '{root}' 디렉토리가 이미 존재하며 비어있지 않습니다.")
        overwrite = input("   내용을 덮어쓰시겠습니까? (y/N): ").lower()
        if overwrite != 'y':
            print("창조 작업을 중단합니다.")
            return False
...
if __name__ == "__main__":
    project_name = "VIBE_FORGE_OS_v5.0"
    try:
        if create_project(project_name):
            install_choice = input("\n라이브러리를 자동으로 설치하시겠습니까? (Y/n): ").lower()
            if install_choice != 'n':
                if install_dependencies(Path(project_name)):
                    print("\n✅ 모든 준비가 완료되었습니다!")
                    print("이제 다음 명령을 실행하여 시스템을 부팅하십시오:")
                    print(f"1. cd {project_name}")
                    print("2. python run.py")
            else:
                print("\n설치를 건너뜁니다. 다음 단계를 수동으로 진행해주세요:")
                print(f"1. cd {project_name}")
                print("2. pip install -r requirements.txt")
                print("3. python run.py")
    except KeyboardInterrupt:
        print("\n\n창조 작업이 사용자에 의해 중단되었습니다.")
```
